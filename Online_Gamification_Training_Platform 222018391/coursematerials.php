...
<body bgcolor="gray">
  <!-- Header section -->
  <header>
    <h1 class="text-center text-white" style="font-family: impact; font-size: 50px; text-shadow: 1px 1px 15px blue; -webkit-text-stroke:0.1px white;">COURSEMATERIALS
  </header><br>
  <!-- Search form -->
...
<section>
    <h1>Course Materials Form</h1>
    <form method="post" action="coursematerials.php">
        ...
        <input type="text" id="material_type" name="material_type" required><br><br>
        <label for="material_url">Material URL:</label>
        <input type="text" id="material_url" name="material_url" required><br><br>
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['insert'])) {
        // Insert section
        $stmt = $connection->prepare("INSERT INTO coursematerials (material_id, course_id, material_name, material_type, material_url) VALUES (?, ?, ?, ?, ?)");
        $stmt->bind_param("iisss", $material_id, $course_id, $material_name, $material_type, $material_url);
    ...
    <!-- Displaying Table of Course Materials -->
    <center><h2>Table of Course Materials</h2></center>
    <table>
        <tr>
            <th>Material ID</th>
            <th>Course ID</th>
        ...
        // SQL query to fetch data from the coursematerials table
        ...
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                ...
                echo "<tr>
                    ...
                    <td>" . $row["material_url"] . "</td>
                    <td><a href='delete_coursematerials.php?material_id=$material_id'>Delete</a></td> 
                    <td><a href='update_coursematerials.php?material_id=$material_id'>Update</a></td> 
                </tr>";
            }
        } else {
