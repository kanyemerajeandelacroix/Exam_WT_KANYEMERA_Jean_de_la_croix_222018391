...
<body bgcolor="gray">
  <!-- Header section -->
  <header>
   <h1 class="text-center text-white" style="font-family: impact; font-size: 50px; text-shadow: 1px 1px 15px blue; -webkit-text-stroke:0.1px white;"> ENROLLMENTS
  </header><br>
  <!-- Search form -->
...
<section>
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['insert'])) {
        // Insert section
        $stmt = $connection->prepare("INSERT INTO enrollments (enrollment_id, student_name, course_id) VALUES (?, ?, ?)");
        $stmt->bind_param("iss", $enrollment_id, $student_name, $course_id);
    ...
    <!-- Displaying Table of Enrollments -->
    ...
    <table>
        ...
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                $enrollment_id = $row["enrollment_id"]; // Removed extra space after field name
                echo "<tr>
                    <td>" . $row["enrollment_id"] . "</td>
                    <td>" . $row["student_name"] . "</td>
                    <td>" . $row["course_id"] . "</td> 
                    <td><a href='delete_enrollments.php?enrollment_id=$enrollment_id'>Delete</a></td> 
                    <td><a href='update_enrollments.php?enrollment_id=$enrollment_id'>Update</a></td> 
                </tr>";
            }
        } else {
